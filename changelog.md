...
- `FlightRecorderMiddleware`: new `observe` mode (accumulates the tape and fires `storageCallback` like `record`, but never writes a fixture file) and an `init()` `storageCallback` hook invoked after each interaction in both `record` and `observe` modes; `_saveSnapshot()` is now `public` so subclasses can persist snapshots elsewhere; unknown `mode` values now throw `FlightRecorder.InvalidMode` at construction instead of falling through to replay. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))

- **Structured output on `ClaudeService.chatStream()`**: `.schema()` combined with streaming previously did nothing on Claude — no forced tool was injected and the caller silently got prose. The stream path now injects the same `structured_output` forced tool + `tool_choice` that sync `chat()` uses, accumulates the block's `input_json_delta` fragments separately from the real tool-call loop (so `beforeToolCall`/execution/a follow-up turn never fire for it), and at end of stream populates the schema, emitting it on the normalized envelope as `choices[].delta.structured_output` and returning it from `chatStream()`. Fails loud with `StructuredOutputError` (including the `stop_reason`) if the block never arrives, rather than degrading to prose. Three small public helpers — `newStructuredOutputStreamState()`, `accumulateStructuredOutputChunk()`, `finalizeStructuredOutputStream()` — hold no reference to the surrounding stream loop, so other SSE providers using the same forced-tool trick can adopt them as-is. ([#198](https://github.com/ortus-boxlang/bx-ai/issues/198))
...
- **Flight recorder now tapes BINARY streaming bodies**: `_wrapStreamLLMCall()` gated raw-body recording on `isSimpleValue()`, but a BoxLang binary (`byte[]`) — exactly what Bedrock's `InvokeModelWithResponseStream` transport returns — is not a simple value, so a binary event-stream body fell into the emit-based branch, was never taped, and replay then silently fell through to a live request. Binary bodies are now recorded base64-encoded behind an explicit `"encoding": "base64"` marker and decoded with `toBinary()` on replay, so the caller gets a byte-equal binary back. String bodies are unchanged. ([#248](https://github.com/ortus-boxlang/bx-ai/issues/248))
...
- **Flight recorder no longer corrupts streaming provider calls**: with Bedrock's and Cohere's stream transports now routed through `wrapLLMCall`, `FlightRecorderMiddleware` treated them as chat round trips — `record` json-serialised a raw binary event-stream body into the fixture, and `replay` handed the recorded chat struct back to a transport that either regex-parses a raw body (Bedrock: crash) or emits through the context and discards the return (Cohere: a silently empty stream). `wrapLLMCall` now branches on `context.stream == true`: a raw-body transport (simple-value return) is taped as `{ "type": "llm", "stream": true, "response": "<raw body>" }` and replayed verbatim; an emit-based transport (`emitSSEChunk`, non-simple return) is passthrough-only with nothing taped, logged once per recorder so the gap isn't silent. In replay, a stream call with no matching `stream: true` interaction falls through to the real handler instead of throwing `ReplayExhausted` or returning a struct. ([#248](https://github.com/ortus-boxlang/bx-ai/issues/248))

- **Lenient structured-output parsing**: `populateStructuredOutput()` only stripped a markdown fence and then handed the raw string to `jsonDeserialize()`, so a reply like `Here is the result: { ... }` blew up with a JSON parse error on every provider without a native structured-output channel (OpenAI-shaped, Gemini, Cohere, Ollama). It now falls back to the same lenient `JSONExtractorTransformer` already wired into `returnFormat: "json"`, recovering prose-wrapped and partially-fenced JSON. Content is parsed once up front and passed to `SchemaBuilder` as a native value, so providers that already hand over clean JSON (Claude/Bedrock re-serialise the `tool_use` input) are unaffected. When nothing parseable can be recovered it throws `StructuredOutputError` instead of a raw deserialization error — decided by running the extractor in `strictMode` + `returnRaw` (so "found nothing" is a `JSONExtractor.NoJSONFound` exception) rather than by treating an empty struct as failure, which mis-rejected a legitimate `Here you go: {}`. ([#198](https://github.com/ortus-boxlang/bx-ai/issues/198))
...
- **Flight recorder replay can no longer cross the streaming/sync boundary or replay the wrong provider**: a recorded `stream: true` interaction is a raw provider body, but the sync `wrapLLMCall` replay matched on `type` alone and would hand that body back as if it were a parsed chat response — strict replay now throws `FlightRecorder.TypeMismatch` and lenient replay skips to the next real chat interaction. Stream replay had the mirror-image problem: it took the next `stream: true` entry regardless of who recorded it, so a Cohere call could consume Bedrock's event-stream body. Stream interactions are now taped with the `transport` marker from the call context and matched on it plus a model+message-count fingerprint of the recorded `request`, and a tape recorded before the marker existed still replays. Finally, an unmatched stream call under `strict: true` now throws `FlightRecorder.StreamReplayUnsupported` instead of quietly making a live call — a strict replay promises zero network — while `strict: false` keeps the previous passthrough. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))

- **`storageCallback` received a live view of the tape, not a snapshot**: `_fireStorageCallback()` passed `variables._tape.copy()`, a shallow copy sharing the SAME `interactions` array, so a callback that persisted "the tape as of interaction 3" was actually holding an object that kept growing behind its back. It now passes a `duplicate()`. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))
...
- **`afterLLMCall` could not rewrite a streamed Claude structured output**: `ClaudeService.chatStream()` emitted the populated structured output to the caller's callback and only then fired `afterLLMCall`, so an `OutputGuard`-style middleware that rewrote `streamState.structuredOutput` changed the returned value while the stream had already carried the original. The hook now fires first, against a named context struct, and both the emitted chunk and the return value come from the (possibly mutated) `streamState.structuredOutput`. ([#198](https://github.com/ortus-boxlang/bx-ai/issues/198))

- **Streamed Claude structured output mishandled a second block and a seeded one**: `accumulateStructuredOutputChunk()` never reset its buffer, so a second `structured_output` `content_block_start` in one stream concatenated the two blocks' fragments into unparseable `{…}{…}`; and `finalizeStructuredOutputStream()` treated a non-empty `input` on `content_block_start` and the streamed fragments as either/or, silently discarding the seed. A new block now resets the accumulator, and a seed plus fragments are merged — the fragments' keys winning where both set one, or stitched onto the seed's object when the fragment is a continuation rather than a document of its own. (`isJSON()`/`jsonDeserialize()` both accept a valid leading token followed by trailing junk, so the merge decides by parsing and requiring a struct.) ([#198](https://github.com/ortus-boxlang/bx-ai/issues/198))

- **A literal `{}` reply no longer logs a bogus extraction warning**: `BaseService.extractJSONContent()` (the `returnFormat: "json"` path) treated "the lenient extractor returned an empty struct" as "no JSON found" and warned on the `ai` log, which mis-reported a model that legitimately answered `{}` or `Here you go: {}`. It now confirms the failure with the extractor's `strictMode` + `returnRaw` mode — the same discrimination `parseStructuredOutputContent()` already used — before warning. In that method, the final `jsonDeserialize()` of the extracted raw string also moved inside the existing `try`, so a string the extractor believed was JSON but that fails to parse surfaces as the documented `StructuredOutputError` rather than a bare parse error from another layer. ([#198](https://github.com/ortus-boxlang/bx-ai/issues/198))

- **`ClosureTool` no longer forces `_chatRequest` on every closure**: `doInvoke()` injected `_chatRequest` into the arguments of every callable, whether or not it wanted it — an extra argument that leaks straight through any closure forwarding its own `argumentCollection`, and into anything serialising its arguments scope. It is now injected only when the callable declares a `_chatRequest` parameter, and a declared one is filtered out of the generated tool schema so the model is never invited to populate it. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))
...
- `FlightRecorderMiddleware`: the five copies of `mode == "record" || mode == "observe"` are now a single `_isCapturing()`. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))

- `FlightRecorderMiddleware` storage callback contract is now documented and cheap: the tape handed to `storageCallback( tape, interaction )` is a point-in-time snapshot whose `interactions` array is new but whose entries are **shared** with the live tape — treat the tape and the interaction as read-only. Previously the whole tape was deep-copied on every interaction, which re-copied every base64 stream body each time. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))
...
- **`FlightRecorderMiddleware` strict replay now throws on unmatched stream calls.** With `strict: true` (the default), a streaming call that has no matching taped interaction throws `FlightRecorder.StreamReplayUnsupported` instead of quietly falling through to a live provider call — a strict replay promises zero network. This bites emit-based stream transports (e.g. Cohere's `emitSSEChunk` seam), whose stream turns are **passthrough-only and record nothing**, so a tape can never contain them. If your fixtures cover streaming for such a provider, either construct the recorder with `strict: false` to keep the old passthrough behaviour, or stop routing those stream calls through replay. Sync (non-stream) replay is unaffected. ([#170](https://github.com/ortus-boxlang/bx-ai/issues/170))
