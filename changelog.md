...
- **HITL suspend/cancel never actually stopped the tool-call chain, and reject never let it gracefully continue**: `OpenAIService.chat()`/`chatStream()` processed tool calls inside nested `.each()` closures, where a `return` on a terminal middleware result (`suspend`/`cancel`/`reject` from e.g. `HumanInTheLoopMiddleware` or `GuardrailMiddleware`) only exited the inner callback — the remaining tool calls in the same batch still executed for real, and the chain still recursed into another `chat()`/`chatStream()` round as if nothing had happened. A suspension never reached `AiAgent.run()`, so `checkpointer.saveState()` was never called and web-mode suspension silently completed instead of pausing.
  Fixed by converting the tool-call loops to explicit, early-exit-capable loops, with two distinct terminal behaviors matching the original (buggy-but-intentional) design: **`suspend`/`cancel` hard-stop** the entire batch immediately and return the terminal `AiMiddlewareResult` to the caller (suspend needs a human to resume it externally; cancel — e.g. `MaxToolCallsMiddleware`'s limit, or HITL's CLI "quit" — means abort the run outright). **`reject` is a soft skip**: only that one tool call is skipped, its rejection reason is appended as the tool's result content, and the batch/chain continues — so a `GuardrailMiddleware` block on one tool call doesn't also prevent unrelated, safe tool calls in the same batch from running, and the caller gets a normal completion (the model responding to the rejection) instead of a raw internal object. `AiAgent.stream()` gained the equivalent suspend/cancel-detection handling that `run()` already had for suspend (streaming termination was previously undetectable at the agent layer); it now short-circuits `afterAgentRun`/`storeInMemory()` uniformly for suspend/cancel while leaving reject to flow through to a normal completion, same as `run()`.
- **`beforeToolCall`/`afterToolCall`/`wrapToolCall` middleware hooks never fired for Claude, Bedrock, or Cohere**: these providers invoked tools directly (`tool.invoke(...)`) instead of going through the middleware pipeline, so `HumanInTheLoopMiddleware` and any other tool-call middleware was a silent no-op outside of OpenAI-family providers. `ClaudeService.executeClaudeTool()`, `BedrockService.executeBedrockTool()`, and `CohereService`'s tool-call loop now fire the same `beforeToolCall`/`wrapToolCall`/`afterToolCall` sequence as `OpenAIService`, with the same suspend/cancel-hard-stop vs. reject-soft-skip split described above. Gemini has no tool-calling implementation yet and is unaffected. Adds `SuspendResumeIntegrationTest` covering suspend-blocks-the-batch, checkpoint save, resume-approve, resume-reject-continues-the-batch, cancel-hard-stops, an unrelated safe tool call surviving a sibling rejection, streaming suspend/cancel/reject, and deterministic credential-free coverage of the newly-wired Claude/Bedrock/Cohere tool-call hooks.
- **Claude/Bedrock/Cohere tool-call middleware context omitted normalized arguments**: `GuardrailMiddleware`'s `argPatterns` guard and `FlightRecorderMiddleware` both resolve tool arguments via `context.toolArgs` or, as a fallback, OpenAI's `toolCall.function.arguments` (a JSON string). Claude/Bedrock carry arguments in `toolCall.input` and Cohere in `toolCall.parameters` — neither matched that fallback, so argument-based guardrails silently saw `{}` for these three providers' tool calls even though `beforeToolCall` now fires correctly. `ClaudeService`, `BedrockService`, and `CohereService` now include normalized `toolName`/`toolArgs` keys in the middleware context. `HumanInTheLoopMiddleware` had the identical shape assumption in two more spots — the suspend payload's `arguments` field (now prefers `context.toolArgs`) and the "edit" resume path, which mutated `toolCall.function.arguments` unconditionally; for Claude/Bedrock/Cohere that wrote to a key the provider never read back, so an operator's edited arguments were silently dropped and the original arguments used instead. The edit-resume path now detects which field actually exists on the tool call (`.function` for OpenAI's JSON-string shape, `.input` or `.parameters` as plain structs otherwise) and mutates the correct one.
