...
- **HITL suspend/cancel/reject never actually stopped the tool-call chain**: `OpenAIService.chat()`/`chatStream()` processed tool calls inside nested `.each()` closures, where a `return` on a terminal middleware result (`suspend`/`cancel`/`reject` from e.g. `HumanInTheLoopMiddleware`) only exited the inner callback — the remaining tool calls in the same batch still executed for real, and the chain still recursed into another `chat()`/`chatStream()` round as if nothing had happened. The terminal result never reached `AiAgent.run()`, so `checkpointer.saveState()` was never called and web-mode suspension silently completed instead of pausing. Fixed by converting the tool-call loops to explicit, early-exit-capable loops that stop the batch and return the terminal result immediately. `AiAgent.stream()` gained the equivalent suspend-detection/checkpoint handling that `run()` already had (streaming suspension was previously undetectable at the agent layer). `AiAgent.run()`'s `storeInMemory()` call is now skipped for any terminal middleware result (suspend/cancel/reject), not just suspend, since a cancelled/rejected run has no assistant turn to store.
- **`beforeToolCall`/`afterToolCall`/`wrapToolCall` middleware hooks never fired for Claude, Bedrock, or Cohere**: these providers invoked tools directly (`tool.invoke(...)`) instead of going through the middleware pipeline, so `HumanInTheLoopMiddleware` and any other tool-call middleware was a silent no-op outside of OpenAI-family providers. `ClaudeService.executeClaudeTool()`, `BedrockService.executeBedrockTool()`, and `CohereService`'s tool-call loop now fire the same `beforeToolCall`/`wrapToolCall`/`afterToolCall` sequence as `OpenAIService`, honoring suspend/cancel/reject and stopping the batch immediately on a terminal result. Gemini has no tool-calling implementation yet and is unaffected. Adds `SuspendResumeIntegrationTest` covering suspend-blocks-the-batch, checkpoint save, resume-approve, resume-reject, streaming suspend, and deterministic credential-free coverage of the newly-wired Claude/Bedrock/Cohere tool-call hooks.
- **`AiAgent.stream()` only short-circuited on a suspended result, not on cancel/reject**: unlike `run()`, a cancelled or rejected tool call mid-stream fell through to fire `afterAgentRun`/`afterAIAgentRun` as if the run had completed normally. `stream()` now mirrors `run()`'s uniform terminal-result check — it returns immediately on any terminal `AiMiddlewareResult`, checkpointing only when it's a suspension.
