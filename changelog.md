...
- **Tools and schema-typed structured output silently dropped on non-Claude Bedrock models**: `params.tools` and a schema-typed `returnFormat` are only wired into the request for model families whose Bedrock request transform actually supports them. Titan/Llama/Mistral/Cohere/OpenAI-compatible have no tool-use transform on this provider and previously silently ignored `params.tools`/structured output, producing a response that looked normal but never used the tool/schema — `chat()`/`chatStream()` now throw `UnsupportedProviderCapability` naming the detected family for those. `claude` and `ai21`/unrecognized model IDs are unaffected (both route through the same Claude-style request builder, which does emit `tools`), so the gate no longer over-fires for them.
