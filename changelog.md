...
- **`request` as a variable/parameter name silently resolves to BoxLang's ambient request scope, not your value** — discovered while building the Slack channel (`SlackGateway.requestHumanInteraction( required HumanInteractionRequest request, ... )`): even `arguments.request` (function param) and `var request = ...` inside a closure (TestBox `it()` bodies) get shadowed by the built-in scope, so `request.getId()` etc. silently operate on the wrong object (returning `null` from missing methods, or throwing "Error getting method [x] for class [RequestScope]") instead of erroring clearly at the assignment. Not a bug fix in bx-ai core itself, but worth documenting here since it cost real debugging time and will bite the next person who names something `request`: don't — use `humanRequest`/`req`/anything else.
