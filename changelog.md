...
- **Bedrock provider parity features** ([#227](https://github.com/ortus-boxlang/bx-ai/issues/227)):
  - **Bearer-token auth**: `configure()` with a plain string, or the `AWS_BEARER_TOKEN_BEDROCK` environment variable, authenticates with a Bedrock long-term API key instead of AWS SigV4 — no AWS credentials needed at all — but only when no AWS access key is available; explicit/env AWS credentials always take precedence over bearer auth. **BREAKING**: `configure( "a-string" )` used to set `modelId`; it now sets `apiKey` per the module-wide "string = apiKey" contract, and a string `apiKey` with no AWS credentials configured now selects bearer auth instead of SigV4.
