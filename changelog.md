...
- **Review fixes (code review of the streaming tool loop)**: `AiAgent.resume()`/`resumeStream()` now run `resolvePendingActions()` (and every other rejecting check) before `clearState()`, so a decision-count mismatch no longer destroys the checkpoint. `CohereService.resumeToolBatch()` shares the follow-up loop with the stream path instead of restarting via `chat()`, emits a terminator for non-simple `returnFormat` results, throws `CohereStreamError` for a nameless streamed tool call, looks tools up via `getTool()` (the `.filter().first()` idiom threw on an empty list), honours `beforeToolCall` argument replacement and HITL edits in pass 2 (`resolveToolArgs()`), and emits a single `middleware_stop` when a later round suspends. `ClaudeService` streaming structured output resets its buffer per block, merges a seeded `input` with deltas, and fires `afterLLMCall` with a named context before delivering; Claude pass 2 honours argument replacement and HITL edits. `FlightRecorderMiddleware` sync replay skips stream interactions, stream replay matches on `transport` plus a frozen request fingerprint (taped packets are duplicated), `strict` replays throw `FlightRecorder.StreamReplayUnsupported` instead of making a live call, and `storageCallback` receives an independent snapshot. `BaseService.extractJSONContent()` no longer writes to the instance scope from its `catch` block (the BoxLang catch-scope rule silently disabled its warning). `ClosureTool` injects `_chatRequest` only when the callable declares it and omits it from the generated schema. ([#271](https://github.com/ortus-boxlang/bx-ai/pull/271))

### ⚠️ Migration

- `FlightRecorderMiddleware` with `strict: true` (the default) now throws `FlightRecorder.StreamReplayUnsupported` for a streamed call that has no matching taped interaction (emit-based transports such as Cohere's cannot be taped); set `strict: false` to keep the previous live passthrough.
