...
- **Bedrock provider parity fixes** ([#227](https://github.com/ortus-boxlang/bx-ai/issues/227)): a batch of correctness fixes to `BedrockService` found while auditing it against the other providers.
  - `onAIChatRequest`/`onAIChatResponse` were announced under legacy names (`onAIRequest`/`onAIResponse`) that nothing else in the module listened for, so listeners (flight recorder, logging, cost tracking) silently missed every Bedrock call — renamed to match every other provider. `onAIError` was already announced on the chat path, but `canRetry` semantics were fixed (see the double-announce fix below) rather than newly added.
  - 429 responses weren't recognized as rate limits — no `onAIRateLimitHit` event, so rate-limit-aware retry/backoff middleware never engaged for Bedrock. Now announced on the chat, streaming, **and embeddings** paths, matching `BaseService`'s contract for chat/streaming (`chatRequest`-keyed payload); embeddings has no `BaseService` equivalent, so it keeps its own `embeddingRequest`-keyed payload.
  - `configure()`'s struct path skipped `super.configure()`, so `settings.defaultParams` and `settings.providers.Bedrock.{params,options}` were never merged into Bedrock's request params — the only provider where module-wide defaults silently didn't apply.
  - Claude-on-Bedrock responses with multiple `"text"` content blocks (interleaved with `thinking`/`tool_use`) only read the first block, silently dropping the rest; now all `text` blocks are joined. This is a Bedrock-specific improvement, not cross-provider parity — the direct (non-Bedrock) `ClaudeService` still only reads the first `"text"` block, tracked as a follow-up.
  - Legacy model families (e.g. Mistral) report token usage only via `X-Amzn-Bedrock-*-Token-Count` response headers, not a response body `usage` field — usage was reported as zero; now backfilled from the headers (tolerating both single-string and array-valued headers).
  - Streaming request/response logging ignored `logRequest`/`logResponse` and always logged, unlike every other provider's streaming path — now gated the same way.
  - The inference-profile/provisioned-model ARN path routed to a non-existent `/application-inference-profile/{arn}/invoke` route with the ARN interpolated unencoded; the only real Bedrock route is `/model/{modelId}/invoke`, where `{modelId}` also accepts an ARN. Fixed to route ARNs through the same path, percent-encoded as a single opaque path segment (agreeing with what `AwsSignatureV4` re-encodes into the canonical request).
  - **Array-shaped message content silently corrupted Titan/Llama/Mistral requests**: these transforms string-concatenated `msg.content` directly, which on an array (image parts, `tool_result`/`tool_use` blocks carried over from Claude-style conversation history) produced BoxLang's array-to-string form instead of usable prose. New `flattenMessageContent()` helper reduces array content to its `text`-typed parts (space-joined, non-text parts skipped) before use in all three transforms; plain-string content is unaffected.
  - **`loadAwsCredentialsFromEnvironment`'s docstring claimed IAM-role support that didn't exist** — only explicit struct credentials and environment variables were ever read. `resolveAwsCredentials()` now implements a real (subset of the) AWS default credential provider chain — see the Added entry below — and the class docblock states exactly what's supported.
  - **A plain-string `apiKey` silently disabled AWS SigV4 signing even when explicit AWS credentials were also configured**: `aiService()`/`aiChat()` inject the module-wide `settings.apiKey`/`BEDROCK_API_KEY` env value into every provider's options as a plain string, including Bedrock's, so `useBearerAuth()`'s old "any non-empty string apiKey ⇒ bearer" check could switch a struct-credentialed request to bearer auth out from under it. Bearer auth is now opt-in only (an explicit `bearerToken` config key or the `AWS_BEARER_TOKEN_BEDROCK` env var — see the Added entry below) and never inferred from `apiKey` at all, so this can no longer happen regardless of what else is configured; explicit/env AWS credentials still always win over bearer auth as additional insurance.
  - **`onAIError` was announced twice for the same chat failure**, once inside `sendBedrockRequest()` with the real computed `canRetry`, and again in `chat()` with a hardcoded `canRetry: true` and a different (incorrect) payload key. Now announced exactly once per failure, aligned with `BaseService.bx`'s `aiRequest`-keyed `onAIError` contract.
  - **`checkGuardrailIntervention()` guarded a null-safe default but then iterated the un-guarded original**, which could throw when a response carried no headers at all. Fixed to iterate the guarded local.
  - Several response-header reads (`retry-after`, the `X-Amzn-Bedrock-*-Token-Count` pair) assumed `httpResult.responseHeader` always exists and headers are always plain strings; now null-guarded, and the token-count headers tolerate an array value (some HTTP clients return multi-valued headers as arrays) by taking the first element.
  - `embeddings()`'s `onAITokenCount` only fired when `promptTokens > 0`, unlike chat's, which fires whenever a `usage` struct exists (even all-zero) — an all-zero-usage embedding call silently got no event. Now matches chat's behavior.
  - `baseURL`'s path prefix (e.g. `https://proxy.internal/bedrock`) was silently dropped — `getBedrockHost()` only ever extracted the host. Now recovered via `getBedrockPathPrefix()` and prepended to the request path for both the wire URL and the path signed by `AwsSignatureV4`.
  - `resolveAwsCredentials()`'s read-modify-write of the auto-resolved credential cache was unguarded against concurrent requests; now wrapped in a per-instance named lock.
  - Two inverted docblocks (`getBedrockPath()`, `AwsSignatureV4.encodeComponent()`) claimed the path handed to `signRequest()` must be raw/unencoded — the code has always correctly passed the already-encoded path, which `signRequest()` intentionally re-encodes a second time to satisfy AWS's non-S3 double-URI-encode canonical-request rule (matches botocore). Docblocks corrected to describe the actual, correct contract; a maintainer "fixing" the code to match the old comments would have broken every signature.
- **Tools and schema-typed structured output silently dropped on non-Claude Bedrock models**: `params.tools` and a schema-typed `returnFormat` are only wired into the request for model families whose Bedrock request transform actually supports them. Titan/Llama/Mistral/Cohere/OpenAI-compatible have no tool-use transform on this provider and previously silently ignored `params.tools`/structured output, producing a response that looked normal but never used the tool/schema — `chat()`/`chatStream()` now throw `UnsupportedProviderCapability` naming the detected family for those. After #226 collapsed `ai21`/unrecognized ids into the `openai` family, `claude` is the only family that passes the gate — which is exactly right, since `transformRequestForClaude()` is the only request builder here that emits `tools`. An unrecognized model ID plus tools now throws instead of quietly building a Claude-shaped request for a model that isn't Claude; opaque `arn:` model IDs still detect as `claude` and are still allowed through.
...
- **`aiGateway()` resolves dotted names as full class paths**: mirroring `aiMemory()`/`aiDocuments()`, a name containing a `.` (e.g. `aiGateway( "myModule.gateways.CustomGateway" )`) is instantiated directly instead of requiring an `onMissingGateway` listener.
- **Bedrock provider parity features** ([#227](https://github.com/ortus-boxlang/bx-ai/issues/227)):
  - **Bearer-token auth**: an explicit `bearerToken` config key, or the `AWS_BEARER_TOKEN_BEDROCK` environment variable, authenticates with a Bedrock long-term API key instead of AWS SigV4 — no AWS credentials needed at all — but only when no AWS access key is available; explicit/env AWS credentials always take precedence over bearer auth. Purely additive: `configure( "a-string" )` keeps its existing, long-standing meaning of setting `modelId`, and bearer auth is never inferred from a generic `apiKey` value.
  - **AWS default credential chain**: `resolveAwsCredentials()` now resolves credentials in the standard AWS order — explicit struct credentials, `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_SESSION_TOKEN` env vars, the bearer token above (short-circuits SigV4 entirely), ECS/EKS container credentials (`AWS_CONTAINER_CREDENTIALS_RELATIVE_URI`/`_FULL_URI`), and EC2 IMDSv2 instance-role credentials — instead of only ever reading explicit/env values. Auto-resolved (container/IMDS) credentials are cached on the instance and refreshed once within 5 minutes of their reported expiration. A failed resolution attempt (nothing anywhere in the chain, e.g. off-EC2 with no role attached) is itself negative-cached for 60 seconds, so callers making frequent requests (e.g. per-text in `sendTitanEmbeddings`) don't pay the container/IMDS network timeout on every single one; if a refresh attempt fails while the previously cached credentials are within their 5-minute expiry buffer but not yet actually expired, those still-valid credentials are reused rather than discarded.
  - **Guardrails + header passthrough**: `providerOptions.guardrailIdentifier`/`guardrailVersion`/`guardrailTrace` map to the `X-Amzn-Bedrock-Guardrail*` request headers; any `providerOptions` key already shaped like `x-amzn-bedrock-*` (or under a `bedrockHeaders` struct) is forwarded verbatim. A Guardrails intervention on the response (header or in-band streaming marker) is logged. Extracted as the pure, directly-testable `buildProviderOptionHeaders()`.
  - **`baseURL` config**: overrides the Bedrock runtime host (VPC endpoint, LocalStack, proxy) for both the request URL and the SigV4 `Host` header, so the two always agree; defaults to the standard regional `bedrock-runtime.{region}.amazonaws.com` endpoint when unset.
  - **Cohere and Titan-v2 embedding shapes**: embeddings previously assumed Titan v1 (`{ inputText }` → `.embedding`) for every model. Now routed by embedding model ID: `cohere.embed-*` batches up to 96 texts per call as `{ texts, input_type }` → `.embeddings[]`; `amazon.titan-embed-text-v2*` uses the v1 body plus optional `dimensions`/`normalize` from request params; everything else keeps the original per-text Titan v1 behavior. Token-usage accounting (and `onAITokenCount`) works uniformly across all three shapes, including Cohere responses that carry no token-count field at all (falls back to the `X-Amzn-Bedrock-Input-Token-Count` response header, then to zero).
