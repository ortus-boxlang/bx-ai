...
- **Bedrock provider parity fixes** ([#227](https://github.com/ortus-boxlang/bx-ai/issues/227)): a batch of correctness fixes to `BedrockService` found while auditing it against the other providers.
  - `onAIChatRequest`/`onAIChatResponse`/`onAIError` weren't announced on Bedrock's chat path, so listeners (flight recorder, logging, cost tracking) silently missed every Bedrock call. Now fired the same as every other provider.
  - 429 responses weren't recognized as rate limits — no `onAIRateLimitHit` event, so rate-limit-aware retry/backoff middleware never engaged for Bedrock. Now announced on both the chat and streaming paths, matching `BaseService`'s contract.
  ...
  - Claude-on-Bedrock responses with multiple `"text"` content blocks (interleaved with `thinking`/`tool_use`) only read the first block, silently dropping the rest; now all `text` blocks are joined.
  - Legacy model families (e.g. Mistral) report token usage only via `X-Amzn-Bedrock-*-Token-Count` response headers, not a response body `usage` field — usage was reported as zero; now backfilled from the headers.
...
- **Tools and schema-typed structured output silently dropped on non-Claude Bedrock models**: `params.tools` and a schema-typed `returnFormat` are only wired into the request for the `claude` model family (Titan/Llama/Mistral/Cohere/AI21/OpenAI-compatible have no tool-use transform on this provider) — previously they were silently ignored, producing a response that looked normal but never used the tool/schema. `chat()`/`chatStream()` now throw `UnsupportedProviderCapability` naming the detected family and pointing at Claude-family models instead of silently no-op'ing.
...
- **Bedrock provider parity features** ([#227](https://github.com/ortus-boxlang/bx-ai/issues/227)):
  - **Bearer-token auth**: `configure()` with a plain string, or the `AWS_BEARER_TOKEN_BEDROCK` environment variable, authenticates with a Bedrock long-term API key instead of AWS SigV4 — no AWS credentials needed at all.
