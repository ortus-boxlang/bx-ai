...
- **`approve_always`/`approve_session` never actually recorded a grant for Slack or HTTP — the two async gateways they were built for**: found during a strict design review of the HITL flow. Two compounding gaps: (1) `HumanInTheLoopMiddleware.gatewayApprove()` built its `GatewayContext` with no identity at all (`context.userId` doesn't exist anywhere below `AiAgent.run()`'s own local scope — no provider's tool-call context ever carried it), so every grant check saw an empty identity and silently no-op'd; (2) the async resume path (`beforeToolCall()`'s `resumeCtx.resumeDecision` branch, what actually fires when `agent.resume()` is called after a Slack button click or HTTP decision POST) only recognized `"reject"`/`"edit"` and treated everything else — including `"approve_always"`/`"approve_session"` — as plain approve, without ever calling `HumanInteractionCoordinator.resolve()`. The synchronous path (`CliGateway`, scripted `MockGateway`) was never affected, which is why Phase 1's own tests didn't catch it. Fixed: `AiBaseRequest`/`AiChatRequest` gain a `userId` property (mirroring the existing `tenantId`, set from `options.userId` — the same identifier `AiAgent.run()` already uses for memory correlation), so `gatewayApprove()` can read `context.chatRequest.getUserId()`; the resume path now builds a real `HumanInteractionDecision` and routes it through `coordinator.resolve( suspendData.suspensionID, decision, tool )` — reusing the *same* `decisionToMiddlewareResult()` the synchronous path already uses, so a resumed decision is dispatched identically regardless of which path produced it. This only records a grant when the coordinator that suspended the run is still the one resolving it (e.g. an application-scoped agent/middleware) — if it isn't (`SuspensionNotFound`), resume still dispatches the decision correctly, just without persisting a grant, rather than failing. `AiAgent.resume()`/`resumeStream()` gain optional `decidedBy`/`reason` parameters so that identity survives the round trip too. New integration test drives a full suspend → `agent.resume( "approve_always", ... )` → a *separate*, later run for the same identity/tool and proves it auto-approves without ever presenting to the gateway again.
...
- **`HumanInTheLoopMiddleware` now defaults its `IDecisionStore` from application-wide settings instead of never having one**: previously `new HumanInteractionCoordinator()` was constructed with no store at all, so `approve_always` silently degraded to session-only behavior no matter what — there was no way to wire a durable store in. New `settings.hitl.decisionStore` block in `ModuleConfig.bx` (`{ provider: "cache", config: {} }`, same shape as the existing `memory` setting) is the application-wide default, resolved via `aiDecisionStore()` when called with no arguments (mirroring how `aiMemory()` already defaults from `settings.memory`). `HumanInTheLoopMiddleware` gains an optional `decisionStore` constructor argument that overrides the default when supplied. Deliberately **no per-agent override** — an application only ever attaches one `HumanInTheLoopMiddleware` at a time, so there's exactly one place this needs configuring: globally, with an explicit override at the point that one middleware is constructed.
