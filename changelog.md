...
- **`returnFormat: "json"` / `asJson()` mis-extracted JSON in two edge cases**: markdown-fence detection matched `` ```json `` anywhere in the reply, including inside a JSON string value or a bare prose mention, so `{"code":"```json x ```"}` and 'I will reply in ```json``` format: ```json {"a":1} ```' both returned `{}`; and the brace/bracket scanner only ever tried the first `{`/`[`, so `'Use {curly} like: {"a":1}'` also returned `{}`. Fence extraction is now parse-validated — each `` ```json `` occurrence is tried in turn and only kept if it deserializes as JSON, otherwise falling through to scan the original untouched text — and the brace/bracket scan retries at each successive `{`/`[` until one parses. A reply that still yields nothing usable now logs a `warn` (previously silent) before degrading to an empty struct; it still never throws. [#222](https://github.com/ortus-boxlang/bx-ai/issues/222)
